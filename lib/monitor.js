...
function Monitor(config) {
  ...
  if (process.env.VCAP_APPLICATION) {
    ...
    this.apiUrl = "https://" +VCAP_APPLICATION.application_uris[0] + "/api";
  } else {
    // LOCAL
...
Monitor.prototype.findChecksNeedingPoll = function(callback) {
  var options = url.parse(this.apiUrl + '/checks/needingPoll');
  this.applyApiHttpOptions(options);
  var self = this;
  https.get(options, function(res) {
    if (res.statusCode != 200) {
      return callback(new Error(self.apiUrl + '/checks/needingPoll resource responded with error code: ' + res.statusCode));
    }
    var body = '';
    ...
    callback(new Error(self.apiUrl + '/checks/needingPoll resource not available: ' + e.message));
  });
};
...
Monitor.prototype.pollCheck = function(check, callback) {
  ...
  var details = {};
  try {
    Poller = this.pollerCollection.getForType(check.type || 'https');
...
Monitor.prototype.declarePoll = function(check, callback) {
  var options = url.parse(this.apiUrl + '/check/' + check._id + '/test');
  options.method = 'PUT';
  this.applyApiHttpOptions(options);
  var self = this;
  var req = https.request(options, function(res) {
    if (res.statusCode != 200) {
      return callback(new Error(self.apiUrl + '/check/:id/test resource responded with error code: ' + res.statusCode));
    }
    res.on('data', function(chunk) {
      // do nothing
    });
    ...
    callback(new Error(self.apiUrl + '/check/:id/test resource not available: ' + e.message));
  });
  req.end();
};
...
Monitor.prototype.createPing = function(error, check, timestamp, time, details, callback) {
  ...
  var options = url.parse(this.apiUrl + '/pings');
  options.method = 'POST';
  options.headers = {
  ...
  var req = https.request(options, function(res) {
    if (res.statusCode != 200) {
      return callback(new Error(self.apiUrl + '/pings resource responded with error code: ' + res.statusCode));
    }
    var body = '';
    res.on('data', function(chunk) {
    body += chunk;
    ...
    callback(new Error(self.apiUrl + '/pings resource not available: ' + e.message));
  });
  req.write(postData);
  req.end();
};
